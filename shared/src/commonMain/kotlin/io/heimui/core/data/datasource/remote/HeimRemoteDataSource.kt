...
/**
 * HTTP methods a server-driven payload is allowed to request. Anything else is refused so a
 * hostile payload cannot coerce the client into issuing, say, a DELETE against an arbitrary path.
 */
...
internal class HeimRemoteDataSource(
    ...
    private suspend fun performFetch(
        ...
        val response = httpClient.get(buildUrl("/screens/$screenId")) {
            headers {
                authTokenProvider?.invoke()?.let { append(HttpHeaders.Authorization, it) }
            ...
            HttpStatusCode.OK -> {
                ...
                if (rawBytes.size > HeimPayloadGuard.MAX_PAYLOAD_BYTES) {
                    ...
                    return RemoteScreenResponse.Error(
                        statusCode = 413,
                        message = "Payload exceeds maximum allowed size of " +
    ...
    suspend fun submitForm(
        ...
        return try {
            val response = httpClient.request(secureUrl) {
     ...
     *
     * The `Authorization` header is attached to whatever URL this returns, so an unchecked
     * absolute endpoint is a direct session-token exfiltration channel. Host, port and scheme
     * must all match the configured origin (or an explicitly allow-listed host).
     */
    private fun resolveSecureUrl(endpoint: String): String {
        ...
        val target = runCatching { Url(endpoint) }.getOrElse {
            throw HeimSecurityException("Malformed form endpoint: '$endpoint'")
        }
        val base = runCatching { Url(baseUrl) }.getOrElse {
            throw HeimSecurityException("HeimUI is misconfigured: baseUrl '$baseUrl' is not a valid URL")
        }
        ...
        val hostAllowed = target.host.equals(base.host, ignoreCase = true) ||
